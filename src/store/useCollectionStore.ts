...
interface CollectionStoreState {
  ...
  bookCollections: Record<string, string[]> // Map of bookId -> [collectionIds]
  isLoading: boolean
}
...
interface CollectionStoreActions {
  ...
  createNewCollection: (collectionData: Omit<Collection, 'id' | 'type' | 'createdAt' | 'isDefault'>) => Promise<Collection>
  ...
  getBookCount: (collectionId: string, library: Book[]) => Promise<number>

  // Book-Collection relationships
  addBook: (bookId: string, collectionId: string) => Promise<void>
  removeBook: (bookId: string, collectionId: string) => Promise<void>
  loadBookCollections: (bookId: string) => Promise<string[]>

  // Toggle book in collection (add if not present, remove if present)
  toggleBookInCollection: (bookId: string, collectionId: string) => Promise<void>

  // Getters
  getActiveCollection: () => Collection | undefined
  getCustomCollections: () => Collection[]
  getSmartCollections: () => Collection[]
  isBookInCollection: (bookId: string, collectionId: string) => boolean
  getBookCollectionIds: (bookId: string) => string[]
}
...
export const useCollectionStore = create<CollectionStore>((set, get) => ({
  ...
  reorderCollections: async (collectionIds) => {
    try {
      const updatedCollections = await reorderCollections(collectionIds)
      set({ collections: updatedCollections })
    } catch (error) {
      console.error('[CollectionStore] Error reordering collections:', error)
      throw error
    }
  },

  getBookCount: async (collectionId, library) => {
    try {
      return await countBooksInCollection(collectionId, library)
    } catch (error) {
      console.error('[CollectionStore] Error counting books:', error)
      return 0
    }
  },
